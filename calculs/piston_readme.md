...
1. Open `piston-tet4.geo` with `gmsh`:
...
1. Click on `3D`, the volume mesh should appear.  
      
1. In the `File` drop-down menu, choose `Save Mesh`, the mesh is saved in the file `piston-tet4.msh`. **The mesh format `msh v2.2` must be used.**
...
*   Part `USER PART: Import mesh, boundary conditions and material`:  
    ...
    *   The variable `eltype` gives the element type used in the program: 4 for 4-node tetrahedral. Other types of elements are possible, see the `silex\`lib\`gmsh.py` library to have the full list.
    ...
    *   The routine `ReadGmshElements` of the library `silex\`lib\`gmsh` is used to import the connectivity table from the mesh file in the table `elements`, the corresponding node numbers are in the table `Idnodes`.  
...
*   A new possible analysis : [Landing gear fork](sujet-tp-english.html)
------------------------------------------------------------------------